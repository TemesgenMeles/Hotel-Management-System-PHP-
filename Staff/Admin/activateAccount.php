...
<div><!-- section -->
            <div class="row"><!-- row -->
                ...
                <div class="col-md-9"><!-- colomn -->
                    <h4>Activate & Deactivate Worker Account</h4>
                    <div><!-- table -->
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <form action="viewWorker.php" method="POST">
                                        ...
                                        <td><button type="submit" name="view" class="btn btn-primary"><span class="">icon</span>View</button></td>
                                    </form>
                                    <?php
                                        if ($row['status'] == 1) {
                                            ?>
                                    <form action="activateDiactivate.php" method="POST">
                                        <input type="text" name="emp_id" value="<?php echo $row['emp_ID']; ?>" hidden>
                                        <td><button type="submit" name="activate" class="btn btn-success"><span class="">icon</span>Activate</button></td>
                                    </form>
                                    <?php
                                        }else{
                                            ?>
                                    <form action="activateDiactivate.php" method="POST">
                                        <input type="text" name="emp_id" value="<?php echo $row['emp_ID']; ?>" hidden>
                                        <td><button type="submit" name="deactivate" onclick="deleteconformation()" class="btn btn-danger"><span class="">icon</span>Diactivate</a></td>
                                    </form>
                                            <?php
                                        }
                                    ?>
        ...
        <script>
            function deleteconformation(){
                var a = confirm('are you sure? you want to block the employee from using the system');
                if(a == false){
