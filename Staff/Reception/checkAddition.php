...
if(isset($_POST['bank_check'])){
    ...
    if($numrow != 0){
        $reservaton_select = "SELECT customer_ID,bank_check from room_reservation where ID = '$reserv_ID'";
        ...
        if ($reservaton_row['bank_check'] == 0) {
            $update_sql = "UPDATE room_reservation set transaction = NULL , bank_check = 1";
            $update_result = mysqli_query($db,$update_sql);
            if ($update_result) {
                $_SESSION['bank_msg'] = "The customer uplode used transaction picture";
            ...
            $delete_sql = "DELETE from room_reservation where ID = '$reserv_ID'";
            $delete_result = mysqli_query($db,$delete_sql);
            if ($delete_result) {
                $_SESSION['bank_msg'] = "The system drops the customer room reservation";
                header("Location: ../../Staff/Reception/reservaationRoom.php");
            }
        }
            ...
            <div class="row container">
                <div class="panel panel-success">
                    <div class="panel-heading">
                        ...
                        $img = "SELECT * from room_reservation where ID = '$reserv_ID'";
                        $img_result = mysqli_query($db,$img);
                        $img_row = mysqli_fetch_array($img_result);
                        $customer_id = $img_row['customer_ID'];
                        
                        ?>
                        <img src="<?php echo 'data:image/jpeg;base64,'. base64_encode($img_row['transaction']).'';?>" alt="image from database" style="width: 400px;" >
                    </div>
                    <form class="form-horizontal" action="insertIntoBank.php" method="post">
                        <input type="text" name="type" value="room_reservation" hidden>
